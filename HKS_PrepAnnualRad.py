...
if len(_selectedSkyMtx) != 0:
    cumSky_radiationStudy = _selectedSkyMtx
else:
    cumSky_radiationStudy = []


def runAnalyses(testPoints, ptsNormals, meshSrfAreas, analysisSrfs, contextSrfs, contextTransmit, parallel,
                cumSky_radiationStudy, viewPoints_viewStudy, viewFields_Angles_D, sunVectors_sunlightHour,
                conversionFac, northVector, lb_preparation, lb_mesh, lb_runStudy_GH):
    RADIANCE_radiationStudy = []
    if len(RADIANCE_radiationStudy) != 0:
        pass
    elif cumSky_radiationStudy != None and (
            len(cumSky_radiationStudy) == 456 or len(cumSky_radiationStudy) == 1752) and analysisSrfs:
        ...
        if contextTransmit != []:
            ...
            if len(cumSky_radiationStudy) == 145:
                radResults, totalRadResults, intersectionMtx = lb_runStudy_GH.parallel_radCalculator(testPoints,
                                                                                                     ptsNormals,
                                                                                                     meshSrfAreas,
                                                                                                     joinedAnalysisMesh,
                                                                                                     contextGroup,
                                                                                                     parallel,
                                                                                                     cumSky_radiationStudy,
                                                                                                     lb_preparation.TregenzaPatchesNormalVectors,
                                                                                                     conversionFac,
                                                                                                     2200000000000000,
                                                                                                     northVector,
                                                                                                     transmitGroup)
                ...
                radResults, totalRadResults, intersectionMtx = lb_runStudy_GH.parallel_radCalculator(testPoints,
                                                                                                     ...
                                                                                                     parallel,
                                                                                                     cumSky_radiationStudy,
                                                                                                     lb_preparation.getReinhartPatchesNormalVectors(),
                                                                                                     conversionFac,
                                                                                                     2200000000000000,
                                                                                                     northVector,
                                                                                                     transmitGroup)
...
def resultVisualization(contextSrfs, analysisSrfs, results, totalResults, legendPar, legendTitle, studyLayerName,
                        bakeIt, checkTheName, l, angle, listInfo, lb_preparation, lb_visualization, runOrientation):
    ...
    # legend geometry
    legendSrfs, legendText, legendTextCrv, textPt, textSize = lb_visualization.createLegend(results, lowB, highB,
                                                                                            numSeg, legendTitle,
                                                                                            lb_visualization.BoundingBoxPar,
                                                                                            legendBasePoint,
                                                                                            legendScale, legendFont,
    ...
    if runOrientation:
        try:
            ...
            pass
    titleTextCurve, titleStr, titlebasePt = lb_visualization.createTitle([listInfo[0]], lb_visualization.BoundingBoxPar,
                                                                         legendScale, customHeading, False, legendFont,
                                                                         legendFontSize, legendBold)
    ...
    if bakeIt:
        ...
        if bakeIt == 1:
            lb_visualization.bakeObjects(newLayerIndex, analysisSrfs, legendSrfs, legendText, textPt, textSize,
                                         legendFont, None, decimalPlaces, True)
        else:
            lb_visualization.bakeObjects(newLayerIndex, analysisSrfs, legendSrfs, legendText, textPt, textSize,
                                         legendFont, None, decimalPlaces, False)
...
def main(north, geometry, context, gridSize, disFromBase, contextTransmit, orientationStudyP, cumSky_radiationStudy,
         legendPar, parallel, runIt, bakeIt, workingDir, projectName):
    # import the classes
    ...
    if len(geometry) != 0 and disFromBase:
        ...
        ## extract test points
        testPoints, ptsNormals, meshSrfAreas = lb_mesh.parallel_testPointCalculator(analysisSrfs, float(disFromBase),
                                                                                    parallel)
        ...
        print
        "Please connect the geometry and set up both the gridSize and the distance from base surface..."
    ...
    try:
        legendTitles[0] = cumSky_radiationStudy[3]
    except:
        pass
    ...
    if runOrientation and cumSky_radiationStudy != []:
        ...
        for angle in range(len(angles) - 1):
            rotationT = (rc.Geometry.Transform.Rotation(math.radians(angles[angle + 1] - angles[angle]),
                                                        rc.Geometry.Vector3d.ZAxis, rotationBasePt))
            ...
            results, eachTotalResult, listInfo, intersectionMtx = runAnalyses(testPoints, ptsNormals, meshSrfAreas,
                                                                              analysisSrfs, mergedContextSrfs,
                                                                              contextTransmit, parallel,
                                                                              cumSky_radiationStudy,
                                                                              viewPoints_viewStudy, viewFields_Angles_D,
                                                                              sunVectors_sunlightHour, conversionFac,
                                                                              northVector, lb_preparation, lb_mesh,
                                                                              lb_runStudy_GH)
        ...
        if legendPar == [] or (legendPar[0] == None and legendPar[1] == None):
            ...
            for key in orirntationStudyRes.keys():
                ...
                if legendPar == [] or legendPar[4] == None:
                    allBuildingsAndContext.extend(
                        [orirntationStudyRes[key]["analysisSrf"], orirntationStudyRes[key]["contextSrf"]])
            ...
            if (bakeIt != 0 or angles[angle + 1] == angles[-1]) and results != -1:
                ...
                for i in range(len(results)):
                    if results[0] != [] and results[i] != None:
                        ...
                        # so confusing!
                        resultColored[i], legendColored[i], l[i], legendBasePoint = resultVisualization(
                            mergedContextSrfs, analysisSrfs,
                            results[i], eachTotalResult[i], legendPar, legendTitles[i],
                            studyLayerNames[i], bakeIt, CheckTheName, l[i], angles[angle + 1], listInfo, lb_preparation,
                            lb_visualization, runOrientation)
                        resV += 1
        ...
        results, totalResults, listInfo, intersectionMtx = runAnalyses(testPoints, ptsNormals, meshSrfAreas,
                                                                       analysisSrfs, contextSrfs, contextTransmit,
                                                                       parallel, cumSky_radiationStudy,
                                                                       viewPoints_viewStudy, viewFields_Angles_D,
                                                                       sunVectors_sunlightHour, conversionFac,
                                                                       northVector, lb_preparation, lb_mesh,
                                                                       lb_runStudy_GH)
    ...
    if results != -1 and len(results) == 4:
        ...
        if not runOrientation:
            totalResults = [
                totalResults]  # make a list of the list so the same process can be applied to orientation study and normal run
        else:
            bakeIt = 0
        ...
        for i in range(len(results)):
            if results[i] != None:
                # angles
                # Add an option for orientation study
                resultColored[i], legendColored[i], l[i], legendBasePoint = resultVisualization(contextSrfs,
                                                                                                analysisSrfs,
                                                                                                results[i],
                                                                                                totalResults[0][i],
                                                                                                legendPar,
...
if _runIt:
            ...
            for i, ptList in enumerate(originalTestPoints):
                ...
                for pt in ptList:
                    ...
                    if result != -1 and len(result) != 4:
                        try:
                            radiationResult.Add(radiationResult_flatten[ptCount], p)
                        except:
                            pass
                    ptCount += 1
    ...
    if result == -1 and not (len(_geometry) != 0 and _geometry[0] != None and _disFromBase):
        print
        "Please connect the geometry or the context" + \
        " and set up both the gridSize and the distance from base surface..."
        w = gh.GH_RuntimeMessageLevel.Warning
        ghenv.Component.AddRuntimeMessage(w,
                                          "Please connect the geometry or the context and set up both the gridSize and the distance from base surface...")
